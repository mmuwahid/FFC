...
### Slice 2B-F carry-over (Task 1) + S043 defense-in-depth (Task 4) + admin nav badge (Task 5)

**1 migration this slice (0033 `admin_rpc_revoke_public`).** Live DB now at 33 migrations.

- [x] **Task 1 — MOTM picker + Notes editor in `MatchEntryReview`** (carry-over from slice 2B-F). Promoted `_setEditMotm`/`_setEditNotes` placeholders to real setters. Extended `Sheet` discriminated union with `motm` + `notes` variants. Computed `effectiveMotm` / `effectiveMotmName` / `effectiveNotes` post-guard so cards reflect either the override or the pending row's value. MOTM card gets a Change/Set affordance + combined-roster bottom sheet (gold-tinted active state + Clear MOTM). Notes card now always rendered with Add/Edit affordance + truncated single-line preview + bottom sheet (textarea max 500 + char counter, Save/Clear label by trim outcome). Both sheets local-state-only — diff sent in `p_edits` on Approve via existing `handleApprove` builder. `editMotm` 4-state semantics documented inline. Did NOT extract a shared MotmPicker component — `RefMatchdayPayload` (ref-side) and `PendingPlayerRow[]` + lookup maps (admin-side) shapes differ enough that normalising would cost more than the duplication. Both files now ~660 LOC, well under the 800-LOC guardrail. CSS additions: `.mer-notes-preview` (single-line truncate) + `.mer-motm-list` / `.mer-motm-team` / `.mer-motm-pick` / `.mer-motm-pick--active`. Commit: `8fba2dd`.
- [x] **Task 4 — Migration 0033 `admin_rpc_revoke_public`** (defense-in-depth, S043 follow-up). REVOKE EXECUTE FROM PUBLIC on every admin SECURITY DEFINER RPC. Postgres grants EXECUTE to PUBLIC by default; the explicit GRANT TO authenticated in earlier migrations coexisted with that PUBLIC grant rather than replacing it. After 0033, anon-key callers get `42501 permission denied for function ...` at the PostgREST gate before any PL/pgSQL runs. Audited 35 RPCs whose body calls `is_admin()` or `is_super_admin()` against live `pg_proc`. `accept_substitute` (captain-OR-admin) included — captains are authenticated users. `request_reroll` excluded as a false positive in the LIKE search. Out of scope: `submit_ref_entry` / `get_ref_matchday` (anon-callable; auth via ref token). **Signature drift caught:** first draft hand-typed; rewrote DDL programmatically from `pg_proc.pg_get_function_identity_arguments(oid)` (caught `ban_player` 2-arg → 3-arg evolution, `admin_submit_match_result` arg-order changes). Verified via anon-key curl: `regenerate_ref_token` / `reject_signup` / `admin_drop_pending_match_event` all return `42501 permission denied for function ...` at PostgREST gate; `get_ref_matchday` (anon-allowed) untouched. ACL transitions `{=X/postgres,...}` → `{postgres=X/postgres,...}` (PUBLIC grant gone). Idempotent / safe to re-run. Commit: `992d1bc`.
- [x] **Task 5 — Admin nav badge on Settings → Admin platform row.** User picked placement after a 3-option chip prompt. New `pendingEntriesCount` state in Settings; admin-only `useEffect` gated by `isAdmin` runs cheap `count: 'exact', head: true` query against `pending_match_entries WHERE status='pending'` on mount; cleanup flag prevents stale setState. Badge renders as a `.st-admin-badge` red pill (`#e63349`) with cream digit, 22px high, `min-width: 22px`, 2px ring (`box-shadow`) to lift it off the red parent row. `aria-label` reads `"N pending match ${count === 1 ? 'entry' : 'entries'} awaiting review"` with proper pluralisation. No realtime — count only matters when admin is actively triaging; Settings is rarely open. Realtime subscription deferred until `pending_match_entries` goes through `ALTER PUBLICATION` (Phase 2A). Commit: `d56eea8`.
- [x] **Build verification** after each commit — `tsc -b` EXIT 0 + `vite build` EXIT 0 (PWA precache 11 entries, ~1565 KiB total / 752.78 KB main JS / 131.32 KB CSS at close). ESLint clean on changed files; two pre-existing errors in `Settings.tsx` (lines 120 + 214) confirmed via `git blame` — out of scope. Dev server (`preview_start ffc-dev`) mounted cleanly after each push.
- [x] **3 feature commits + 1 close-out commit, all pushed.** `8fba2dd` Task 1 · `992d1bc` Task 4 · `d56eea8` Task 5 · close-out (this commit).

### S047 gotchas / lessons (additive)

- **Two-layer defense for SECURITY DEFINER RPCs.** Layer 1: helper-function NULL safety (S043 — `COALESCE(...,false)`). Layer 2: REVOKE EXECUTE FROM PUBLIC at the PostgREST gate (S047). Either layer alone shuts out anon, but both together survive future helper bugs and accidental signature regressions. The ACL transition `{=X/postgres,...} → {postgres=X/postgres,...}` is the durable one-line check in `pg_proc.proacl::text` that proves Layer 2 is active.
- **Generate migration DDL from `pg_proc`, not from migration history.** Hand-typing function signatures from S008/S016/S028/etc. headers drifted on `ban_player` (2-arg → 3-arg) and `admin_submit_match_result` (arg-order change). The fix: query `pg_proc.pg_get_function_identity_arguments(oid)` and emit the SQL programmatically. Saves a "function does not exist" fail at apply time. The S024 schema-drift lesson generalises beyond column names to function signatures.
- **`accept_substitute` is captain-OR-admin, not admin-only.** Easy false positive in the "calls `is_admin()`" search. Body shape: `IF NOT EXISTS (captain check) AND NOT is_admin() THEN RAISE` — meaning admins are an alternate path. Both captains and admins are `authenticated`, so `REVOKE FROM PUBLIC + GRANT TO authenticated` preserves the captain path correctly. Worth the second-pass body inspection on RPCs that look gate-mixed.
- **`request_reroll` was a false positive in the `prosrc LIKE '%is_admin%'` search.** Function references "draft session" and other admin-flavoured concepts but doesn't actually call `is_admin()`. Don't trust LIKE-on-source as the audit's final answer; confirm by reading the body. Captain check is sufficient for that RPC.
...
- **`editMotm` 4-state semantics via discriminated `null`.** `null` = no edit, `{x, null}` = set profile, `{null, x}` = set guest, `{null, null}` = explicit clear. The fourth state distinguishes "user actively cleared MOTM" from "user didn't touch it" — matters because `handleApprove` should only send fields the user actually touched.
...
- **Screen-reader copy for count badges:** `"N pending match ${count === 1 ? 'entry' : 'entries'} awaiting review"`. The pluralisation matters; "1 pending match entries" reads broken.
- **Two-track session pattern (carry-over + targeted-from-todo).** S047 ran one S046-carry-over (Task 1 MOTM/Notes) and two from the standing todo (Task 4 defense-in-depth, Task 5 nav badge). Works well for short sessions where blocked items (live device tests, real-Thursday-only verifications) accumulate but other agenda items are independent. Carry-over items get a "Task N" label that links them to the originating slice; new items get standalone task numbers. Future close-outs should preserve this distinction.

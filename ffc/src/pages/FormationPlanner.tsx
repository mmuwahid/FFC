/**
 * §3.19 Formation Planner — Slice A (foundation).
 *
 * Per-team tactical board, shared read-only across the team, editable by
 * the team's captain. This first slice lands the scaffolding:
 ...
 *   • Static pitch SVG with tokens positioned from the selected preset
 *   • Save button wired to upsert_formation(matchday, team, pattern, layout_jsonb)
 *
 * Deferred to later slices:
 *   • Drag-drop tokens + Custom pattern mode
...
export function FormationPlanner() {
  ...
  const slotAssignments = useMemo(() => {
    if (!selectedPreset || myRoster.length === 0) return [] as (RosterEntry | null)[]
    const slots = selectedPreset.slots
    const roster = [...myRoster]
    const assignments: (RosterEntry | null)[] = Array(slots.length).fill(null)
    ...
    for (let i = 1; i < slots.length && roster.length; i += 1) {
      assignments[i] = roster.shift() ?? null
    }
    return assignments
  }, [selectedPreset, myRoster])

  const onSave = useCallback(async () => {
    if (!matchday || !myRosterEntry || !selectedPreset) return
    setSaving(true)
    setError(null)
    const layout = selectedPreset.slots.map((slot, i) => {
    ...
    const args = {
      ...
      p_pattern: selectedPreset.pattern,
    ...
    await loadAll()
  }, [matchday, myRosterEntry, selectedPreset, slotAssignments, loadAll])
    ...
    <div className={`fp-shell fp-shell--${team}`}>
      ...
      <div className="fp-pattern-row" role="tablist" aria-label="Formation pattern">
        ...
        <button
          type="button"
          role="tab"
          aria-selected={pattern === CUSTOM_PATTERN}
          className={`fp-chip fp-chip--custom${pattern === CUSTOM_PATTERN ? ' fp-chip--active' : ''}`}
          disabled
          title="Slice B — drag-drop custom layout"
        >
          <span className="fp-chip-label">Custom</span>
          <span className="fp-chip-tag">Coming soon</span>
        </button>
      </div>
      ...
      <div className="fp-pitch-wrap">
        ...
        <div className="fp-tokens" aria-label="Formation tokens">
          {selectedPreset && selectedPreset.slots.map((slot, i) => {
            const entry = slotAssignments[i]
            const isGk = i === 0
            return (
              <div
                key={i}
                className={`fp-tok fp-tok--${team}${isGk ? ' fp-tok--gk' : ''}${entry ? '' : ' fp-tok--empty'}`}
                style={{ left: `${slot.x}%`, top: `${(slot.y / 150) * 100}%` }}

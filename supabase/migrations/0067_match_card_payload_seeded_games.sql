-- supabase/migrations/0067_match_card_payload_seeded_games.sql
--
-- S063 — fix the share-PNG match number off-by-games_seeded.
--
-- Bug: PNG card showed "Match 2 of 40" while the Matches tab showed
-- "Game 32 / 40" for the same match. Root cause: get_match_card_payload
-- (mig 0049) computes v_match_number as a raw COUNT(*) of non-friendly
-- matchdays in the season — it never adds games_seeded.
--
-- Matches.tsx adds `seedOffset = season.games_seeded ?? 0` to its tab-side
-- counter (line ~210); Season 11 was seeded with 30 prior games before the
-- app started tracking, so the first app-tracked match is "Game 31 / 40",
-- not "Match 1 / 40".
--
-- Fix: add `+ COALESCE(s.games_seeded, 0)` to the match-number derivation.
...
BEGIN
  ...
  -- Pull season-level games_seeded so the match-number can match the
  -- Matches tab's "Game N / Total" labelling.
  ...
  -- Match number = games_seeded + rank of this matchday among non-friendly
  -- matchdays in the same season, ordered by (kickoff_at, id) ascending.
  -- Friendlies excluded so league numbering stays stable.

...
## Current state (S055 close, 29/APR/2026)
- **Phase:** Phase 1 complete. Phase 2A + Phase 2B code-complete (S051). Phase 2 live acceptance still owed on a real Thursday matchday (Atomo handling). **S053** = Phase 3 begins — awards page (V3.0:139) shipped. **S054** = share PNG (V3.0:140) shipped end-to-end + 3 PR merges (Atomo PR #9 unlock_roster, PR #13 Admin Roster Setup, PR #14 admin_add_commitment) + 2 issue fixes (#8 Leaderboard restructure, #12 login+poll flicker) + 4 issues closed. **S055** = Phase 3 backlog refreshed (drop OCR/highlights/streaks; add payment tracker / player badges / injury list — all originally Phase 4 — Extras in V1.0–V2.4) + Atomo's GitHub issue #15 shipped (roster setup pool × button + visible waitlist + hybrid auto-fill).
- **Live:** https://ffc-gilt.vercel.app · `main` clean at `ffbdc46`.
- **Migrations on live DB:** 54 (deliberate gap at 0050 from PR #13's 0049/0050 → 0051/0052 renumber). S053 added 0047 (`season_awards` + view + trigger). S054 added: 0048 (`unlock_roster` RPC, PR #9), 0049 (`get_match_card_payload` RPC + `match-cards` storage bucket, share-PNG), 0051 (`admin_add_guest`, PR #13), 0052 (`admin_update_match_draft`, PR #13), 0053 (`admin_add_commitment`, PR #14). S055 adds: 0054 (`admin_cancel_commitment` + `admin_cancel_guest`, issue #15).

...
**Status:** Phase 1 Steps 1, 2, 3 of V2.8 **COMPLETE**; Step 4 **slices 1–7 COMPLETE** at S026. Slices 1–6 shipped at S022-S025 (Leaderboard · Profile · Rules · Matches · Match Detail · Settings · AdminPlayers · AdminMatches + migration 0016). **Slice 7 (S026)** = **§3.7 Poll Depth-B** (9 states: pre-open · not-voted tri-button · confirmed-#N · waitlisted · guest-unlocked · roster-locked · draft-in-progress · teams-revealed · penalty sheet; realtime on poll_votes/match_guests/matchdays/draft_sessions/match_players; guest invite sheet with 5 required chip groups) + **Leaderboard Depth-B gate** (realtime on matches+match_players · pull-to-refresh with resistance · skeleton rows with shimmer) + **Phase-2 seed `edit_match_players` RPC + UI** (per-player stat correction on Edit result sheet via toggle) + **§3.18 Phase 5.5 Draft-in-progress card** (pulsing amber dot · pick count · picker team · captain · elapsed · stuck-threshold override buttons disabled pending Phase-2 RPCs) + **§3.5 guest-threshold auto-flag trigger** (matchdays.friendly_flagged_at stamped on guest threshold cross per app_settings.guest_friendly_thresholds). S026 shipped 1 commit `45383bc` (+2047/-74 across 8 files) bundling **migrations 0017 · 0018 · 0019**. Live at https://ffc-gilt.vercel.app. **Migrations on live DB: 19 (0001 → 0019_poll_rpcs).** **Dropdown pattern** (`feedback_anchored_dropdowns.md`). **Match Detail pattern** — portal-rendered bottom sheet with optional W/D/L chip via `profileId`. **Admin direct-submit pattern (S025)** — `admin_submit_match_result` creates `matches` + N `match_players` + approves + audits in one tx. **Poll state-machine pattern (S026)** — all 9 Poll states derive from `(now vs poll windows) × myVote × roster_locked_at × draft.status × commitments[].team`; no separate phase enum. **Realtime-capable screen pattern (S026)** — `loadData(mode: 'initial' | 'refresh')` callback shared by initial mount + realtime subscribe + pull-to-refresh; `mode` gates skeleton-clear vs refresh-spinner. **Phase-2 seed pattern (S026)** — low-blast-radius admin affordances (audited, whitelisted, strictly additive) can land ahead of schedule during Phase 1. `supabase/migrations/0012_grants.sql` applied live — future table-creating migrations MUST rely on 0012's DEFAULT PRIVILEGES or emit own GRANTs; RLS alone does not grant access (Supabase "auto table exposure" OFF). All SECURITY DEFINER RPCs need explicit `GRANT EXECUTE ... TO authenticated` (DEFAULT PRIVILEGES from 0012 don't cover functions). `ffc/vercel.json` SPA catch-all rewrite load-bearing. **Schema-drift pattern** — query `information_schema.columns` / `pg_proc.pg_get_function_arguments(oid)` / `pg_type` / `information_schema.views` BEFORE writing PostgREST queries, PL/pgSQL function calls, enum casts, or view-based caller logic. S024 drifts: `matchdays.venue` (not `venue_label`), no `matchday_number`, no `match_players.late_cancel_*`. S025 drifts: `log_admin_action` 4-arg (admin derived via `current_profile_id()`), `team_color` not `team_colour` (plpgsql lazy-parses so CREATE passes but first call fails), `seasons.starts_on` not `started_on`. **S026 drifts:** `profiles.banned_until_matchday_id` doesn't exist (bans live on `public.player_bans` with `starts_at`/`ends_at`/`revoked_at`); `v_match_commitments` view exposes only `guest_display_name` (not `match_guests.id`), so Poll.tsx joins by display_name client-side (view extension queued for S027). Position enum UPPERCASE (`GK`/`DEF`/`CDM`/`W`/`ST`); leaderboard sort `motm` singular; `edit_match_result` narrow (score/MOTM/notes only, requires `approved_at IS NOT NULL`, no nested `players` — use `edit_match_players` for per-player post-approval corrections). `cast_poll_vote(matchday_id, choice)` accepts `'yes'|'no'|'maybe'|'cancel'`; re-voting yes after cancel resets `committed_at` (anti-hoarding per §3.6). `invite_guest` is confirmed-only + 8-arg (display_name + primary + secondary + stamina + accuracy + rating + description); raises `FFC_INVITER_NOT_CONFIRMED` / `FFC_NO_GUEST_SLOT`. Google OAuth in Testing mode — consent shows `hylarwwsedjxwavuwjrn.supabase.co` (Supabase platform limit). Authoritative plan: `planning/FFC-masterplan-V2.8.md`. Next: **S027 — live user acceptance pass on all 5 S026 scope items + captain reroll modal + draft force-complete/abandon RPCs + v_match_commitments view extension**.

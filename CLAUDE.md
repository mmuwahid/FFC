...
**Status:** Phase 1 Steps 1, 2, 3 of V2.8 **COMPLETE**; Step 4 **slices 1–6 COMPLETE** at S025. Slice 1 = §3.13 Leaderboard (S022). Slice 2 = §3.14 Player Profile + §3.16 Rules static + Friendly Game data model (S023). Slices 3–5 = Rules expansion (6 cards) + §3.20 Matches list + §3.15 Match Detail sheet + §3.16 Settings full replacement + AdminPlayers Active/Rejected enrichment (S024). **Slice 6 (S025)** = §3.17 AdminPlayers full (4 tabs · search · edit sheet · ban/unban · reinstate) + §3.18 AdminMatches full CRUD (create/edit matchday · lock roster · result entry · edit result) + **migration 0016** with 8 admin RPCs. S025 shipped 2 code commits — see `sessions/S025/session-log.md`. User full UI acceptance PASSED. Live at https://ffc-gilt.vercel.app. **Dropdown pattern** (`feedback_anchored_dropdowns.md`). **Match Detail pattern** — portal-rendered bottom sheet with optional W/D/L chip via `profileId`. **Admin direct-submit pattern (S025)** — `admin_submit_match_result(matchday_id, score_w, score_b, motm_profile, motm_guest, players jsonb[], notes, approve=true)` creates `matches` + N `match_players` + approves + audits in one tx. `supabase/migrations/0012_grants.sql` applied live — future table-creating migrations MUST rely on 0012's DEFAULT PRIVILEGES or emit own GRANTs; RLS alone does not grant access (Supabase "auto table exposure" OFF). `ffc/vercel.json` SPA catch-all rewrite load-bearing. **Migrations 0013 + 0014 + 0015 + 0016 live** — friendly-game columns + `v_season_standings` excludes friendly · `profiles.push_prefs jsonb` (7 keys) · 8 admin RPCs (`update_player_profile` · `ban_player` · `unban_player` · `reinstate_rejected` · `create_matchday` · `update_matchday` · `lock_roster` · `admin_submit_match_result`). All SECURITY DEFINER RPCs need explicit `GRANT EXECUTE ... TO authenticated` (DEFAULT PRIVILEGES from 0012 don't cover functions). **Schema-drift pattern** — query `information_schema.columns` / `pg_proc.pg_get_function_arguments(oid)` / `pg_type` BEFORE writing PostgREST queries, PL/pgSQL function calls, or enum casts. S024 drifts: `matchdays.venue` (not `venue_label`), no `matchday_number` column, no `match_players.late_cancel_*`. S025 drifts: `log_admin_action` is 4-arg not 5-arg (admin derived via `current_profile_id()`), `team_color` not `team_colour` (plpgsql lazy-parses so CREATE passes but first call fails), `seasons.starts_on` not `started_on`. Position enum UPPERCASE (`GK`/`DEF`/`CDM`/`W`/`ST`); leaderboard sort `motm` singular; `edit_match_result` narrow (score/MOTM/notes only, requires `approved_at IS NOT NULL`, no nested `players` edits). Google OAuth in Testing mode — consent shows `hylarwwsedjxwavuwjrn.supabase.co` (Supabase platform limit). Authoritative plan: `planning/FFC-masterplan-V2.8.md`. Next: **S026 — §3.7 Poll screen Depth-B kickoff + friendly-review test + §3.5 +1 auto-flag trigger**.

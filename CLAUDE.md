...
## Current state (S060 close, 30/APR/2026)
- **Phase:** Phase 1 complete. Phase 2A + 2B code-complete (S051). Phase 2 live acceptance still owed on a real Thursday matchday. **S053** awards page (V3.0:139). **S054** share PNG (V3.0:140) + 3 PR merges. **S055** issue #15 roster setup refinements. **S056** payment tracker spec/migration/screen-skeleton (V3.0:147) + Atomo issue fix-pack. **S057** PR #28+#29 merges + Atomo issue triage + quick-win fixes #24/#26/#27. **S058** = all 4 outstanding GitHub issues closed (#22 #23 #25 #21). **S059** = live ref-access debug cascade + 9-issue GitHub sweep (#30–#38). **S060** = (a) Phase 3 payment tracker (V3.0:147) verified shipped — backend complete since S056 mig 0055, frontend skeleton complete since S056+S058; live-pipeline verification deferred to Thursday matchday. (b) PR #39 triaged + comprehensive workflow-rules comment posted (don't merge — would erase S053–S059); awaiting atomosh response. (c) Issue #41 closed via mig 0064 + 5 frontend touchpoints — `matches.ref_name` TEXT + `pending_match_entries.ref_name` + `pending_match_entry_players.is_no_show` + CREATE OR REPLACE rewrites of `submit_ref_entry` (reads ref_name + per-player is_no_show) and `approve_match_entry` (copies both through pipeline); RefEntry pre-match required ref-name input gated ≥2 chars; RefEntry review "Injured Players" toggle section per team; Matches.tsx renders gold REF pill in banner middle + 🤕→🩹 swap. (d) Issue #38 CRITICAL closed: `.app-topbar-bell` + `.app-topbar-avatar` 36×36→44×44 per Apple HIG. (e) Slop-mockup lesson captured in lessons.md Critical Rule #6 (sharpened with `grep -rn` instruction) + S060 patterns section + auto-memory `feedback_mockup_no_redesign.md`.
- **Live:** https://ffc-gilt.vercel.app · `main` clean at `fb0b542` (S060 re-log docs commit pushes after this). **Open GitHub: 1 PR (#39 awaiting atomosh) + 1 issue (#40 player-name standardization, deferred to S061).**
- **Migrations on live DB:** 64. S053 → 0047. S054 → 0048 / 0049 / 0051 / 0052 / 0053. S055 → 0054. S056 → 0055. S058 → 0056 / 0057 / 0058 / 0059. S059 → 0060 (`seasons_games_seeded` retroactively recorded) / 0061 (`admin_delete_matchday`) / 0062 (cascades through UNAPPROVED drafts) / 0063 (`match_players.slot_index`). **S060 → 0064** (`matches.ref_name` TEXT NULL + `pending_match_entries.ref_name` + `pending_match_entry_players.is_no_show` BOOL NOT NULL DEFAULT false + CREATE OR REPLACE rewrites of `submit_ref_entry` (reads `p_payload->>'ref_name'` + per-player `is_no_show`) and `approve_match_entry` (copies `ref_name` from pending → matches; copies `is_no_show` from `pending_match_entry_players` → `match_players`)).

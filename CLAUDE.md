...
**Status:** Phase 1 Steps 1, 2, 3 of V2.8 **COMPLETE**; Step 4 **slices 1–5 COMPLETE** at S024. Slice 1 = §3.13 Leaderboard (S022). Slice 2 = §3.14 Player Profile + §3.16 Rules static screen + Friendly Game data model (S023). Slices 3–5 = S023 acceptance + Rules expansion (6 cards incl. Kick-off/Cards/Awards) + §3.20 Matches list + §3.15 Match Detail bottom-sheet + §3.16 Settings full replacement + AdminPlayers Active/Rejected enrichment (S024). S024 shipped 5 code commits — see `sessions/S024/session-log.md`. Live at https://ffc-gilt.vercel.app. **Dropdown pattern** (`feedback_anchored_dropdowns.md`) — screen-level controls drop down from their trigger. **Match Detail pattern** — reusable portal-rendered bottom sheet (`components/MatchDetailSheet.tsx`) opens from Matches list (no W/D/L chip) or Profile Recent Matches (W/D/L chip resolved from profile-owner team via `profileId` prop). `supabase/migrations/0012_grants.sql` applied live — every future table-creating migration MUST rely on 0012's DEFAULT PRIVILEGES or emit its own GRANTs; RLS alone does not grant access (Supabase "auto table exposure" OFF on this project). `ffc/vercel.json` ships SPA catch-all rewrite — deleting it will 404 every non-root URL. Migrations 0013 + 0014 + **0015** live: friendly-game columns + `v_season_standings` excludes friendly matchdays + `profiles.push_prefs jsonb NOT NULL DEFAULT` 7-key object (Settings row 2 backing storage; push-delivery backend still Phase 2). **Schema-drift discovery pattern** (S024) — always query `information_schema.columns` BEFORE writing PostgREST embeds; drifts caught include `matchdays.venue` (not `venue_label`), no `matchday_number` column (compute client-side from season matchdays ordered asc), no `match_players.late_cancel_*` columns yet. Position enum is UPPERCASE (`GK`/`DEF`/`CDM`/`W`/`ST`); leaderboard sort enum is `motm` singular. Google OAuth in Testing mode (Client ID `991515683563-ncjuidcn08psinv7oq8jb9kevp4b6g32…`) — consent screen shows `hylarwwsedjxwavuwjrn.supabase.co` not "FFC" (Supabase platform limit, $25/mo Pro fix deferred indefinitely). Authoritative plan: `planning/FFC-masterplan-V2.8.md`. Next: **S025 — dedicated acceptance pass on all S024 output + §3.17 full admin edit/ban flow (migration 0016)**.

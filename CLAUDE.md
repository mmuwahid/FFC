...
**Status:** Phase 1 Steps 1, 2, 3 of V2.8 **COMPLETE**; Step 4 **slices 1+2 COMPLETE** at S023. Slice 1 = §3.13 Leaderboard (S022). Slice 2 = §3.14 Player Profile + §3.16 League Rules static screen + Friendly Game data model (S023). S023 shipped 11 commits — see `sessions/S023/session-log.md`. Live at https://ffc-gilt.vercel.app. **Dropdown pattern** (`feedback_anchored_dropdowns.md`) — screen-level controls drop down from their trigger. `supabase/migrations/0012_grants.sql` applied live — every future table-creating migration MUST rely on 0012's DEFAULT PRIVILEGES or emit its own GRANTs; RLS alone does not grant access (Supabase "auto table exposure" OFF on this project). `ffc/vercel.json` ships SPA catch-all rewrite — deleting it will 404 every non-root URL. Migrations 0013 + 0014 live: `matchdays.friendly_flagged_at`, `matchdays.is_friendly`, `match_players.is_no_show`, no-show `app_settings` keys, `v_season_standings` excludes friendly matchdays. Google OAuth in Testing mode (Client ID `991515683563-ncjuidcn08psinv7oq8jb9kevp4b6g32…`) — consent screen shows `hylarwwsedjxwavuwjrn.supabase.co` not "FFC" (Supabase platform limit, $25/mo Pro fix deferred indefinitely). Authoritative plan: `planning/FFC-masterplan-V2.8.md`. Next: **S024 — user acceptance for S023 work + §3.15 Match Detail**.

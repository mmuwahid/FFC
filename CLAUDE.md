...
## Current state (S059 close, 30/APR/2026)
- **Phase:** Phase 1 complete. Phase 2A + 2B code-complete (S051). Phase 2 live acceptance still owed on a real Thursday matchday. **S053** awards page (V3.0:139). **S054** share PNG (V3.0:140) + 3 PR merges. **S055** issue #15 roster setup refinements. **S056** payment tracker spec/migration/screen-skeleton (V3.0:147) + Atomo issue fix-pack. **S057** PR #28+#29 merges + Atomo issue triage + quick-win fixes #24/#26/#27. **S058** = all 4 outstanding GitHub issues closed (#22 #23 #25 #21). **S059** = live ref-access debug cascade — `admin_delete_matchday` RPC (0061 + 0062 v2 cascading through unapproved drafts) + Delete UI on Roster Setup (Saved-state footer + always-on topbar 🗑) + relaxed AdminMatches Delete-match gate to any matches row + **Poll team-split bug fix** (missing `.eq('match_id', …)` on match_players query was returning rows across every match each player had ever been in) + **slot_index column on match_players** (0063 for stable team-list display order; FOREACH INSERTs in single transaction share `now()`) + Poll polish (hide rank chip when locked, captain restyled as gold filled pill). **S059 sweep (post-original-close)** = 9 GitHub issues closed (#30 #31 #32 #33 #34 #35 #36 #37 #38): WhatsApp share toast feedback, Sign-out duplicate removed, AdminPlayers default tab Active, Leaderboard last-5 grid-template column-order fix + D pill grey, Match cards now show cards/injuries inline, Poll locked-row layout fix, EditRosterPostSheet auto-populate (PostgREST FK ambiguity fix), Match-card alignment + MOTM strip + HAT pill cleanup, UI audit posted as #38 tracking comment + closed.
- **Live:** https://ffc-gilt.vercel.app · `main` clean at `1dd1f27` (S059 re-log docs commit pushes after this). **Open GitHub issues: 0.**
- **Migrations on live DB:** 63. S053 → 0047. S054 → 0048 / 0049 / 0051 / 0052 / 0053. S055 → 0054. S056 → 0055. S058 → 0056 / 0057 / 0058 / 0059. **S059 → 0060** (`seasons_games_seeded` retroactively recorded — already on remote from `bb13127` but missing from local history; `npx supabase migration repair --status applied 0060 --linked`), **0061** (`admin_delete_matchday` SECURITY DEFINER + cascades children + refuses if matches row exists), **0062** (CREATE OR REPLACE — admin_delete_matchday now cascades through UNAPPROVED draft matches; refuses only on `approved_at IS NOT NULL`), **0063** (`match_players_slot_index` — adds `slot_index integer` column + backfill via window function + new index + CREATE OR REPLACE updates to `create_match_draft`/`admin_update_match_draft`/`admin_edit_match_roster` to write slot_index from input position).

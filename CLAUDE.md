...
- **Phase:** Phase 1 complete. Phase 2A + 2B code-complete (S051). Phase 2 live acceptance still owed on a real Thursday matchday. **S053** awards page (V3.0:139). **S054** share PNG (V3.0:140) + 3 PR merges. **S055** issue #15 roster setup refinements. **S056** payment tracker spec/migration/screen-skeleton (V3.0:147) + Atomo issue fix-pack. **S057** PR #28+#29 merges + Atomo issue triage + quick-win fixes #24/#26/#27. **S058** = all 4 outstanding GitHub issues closed (#22 UI audit + Payments CSS regression restore + #25 PNG hero in MatchDetailSheet + friendlies-in-Matches-tab + #23 full notification pipeline coverage + #21 admin match management redesign mockup-first). **S059** = live ref-access debug cascade — `admin_delete_matchday` RPC (0061 + 0062 v2 cascading through unapproved drafts) + Delete UI on Roster Setup (Saved-state footer + always-on topbar 🗑) + relaxed AdminMatches Delete-match gate from approved-only to any matches row + **Poll team-split bug fix** (missing `.eq('match_id', …)` on match_players query was returning rows across every match each player had ever been in) + **slot_index column on match_players** (0063 — for stable team-list display order; FOREACH INSERTs in single transaction share `now()`) + Poll polish (hide rank chip when locked, captain restyled as gold filled pill).
- **Live:** https://ffc-gilt.vercel.app · `main` clean at `e960df2` (S059 close docs commit pushes after this).

...
## Current state (S058 close, 30/APR/2026)
- **Phase:** Phase 1 complete. Phase 2A + 2B code-complete (S051). Phase 2 live acceptance still owed on a real Thursday matchday. **S053** awards page (V3.0:139). **S054** share PNG (V3.0:140) + 3 PR merges. **S055** issue #15 roster setup refinements. **S056** payment tracker spec/migration/screen-skeleton (V3.0:147) + Atomo issue fix-pack. **S057** PR #28+#29 merges + Atomo issue triage + quick-win fixes #24/#26/#27. **S058** = all 4 outstanding GitHub issues closed (#22 UI audit + Payments CSS regression restore + #25 PNG hero in MatchDetailSheet + friendlies-in-Matches-tab + #23 full notification pipeline coverage [scoped match_entry_approved + matchday_created trigger + ranking_changed snapshot] + #21 admin match management redesign with mockup-first [admin_delete_match + admin_edit_match_roster RPCs + 2 new sheets + search on roster-add picker]).
- **Live:** https://ffc-gilt.vercel.app · `main` clean (S058 close docs commit pushes after this).
- **Migrations on live DB:** 58. S053 → 0047. S054 → 0048 / 0049 / 0051 / 0052 / 0053. S055 → 0054. S056 → 0055. **S058 → 0056** (open `get_match_card_payload` to all auth users), **0057** (`notifications_match_lifecycle`: scope match_entry_approved to match's 14 players + `matchday_created` enum + `notify_matchday_created` trigger), **0058** (`ranking_change_notifications`: `ranking_changed` enum + `player_rank_snapshots` table + `snapshot_and_diff_ranks` function + approve_match_entry calls it), **0059** (`admin_match_management`: `admin_delete_match` + `admin_edit_match_roster` RPCs).
